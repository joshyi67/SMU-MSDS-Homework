...
theme(axis.text.x  = element_text(size = 7, angle = 90),axis.text.y  = element_text(size = 7))+geom_tile()
...
##### Showing the significance p-values for each variable. Each column represents different data sets: Training, Test and combined.

```{r BaseGLM}
#TrainDataSet
glm_modeltrain <- glm(emp_train$Attrition~.,emp_train[,c(cols.CatGLM)], family = binomial)
model_Train = data.frame(coef(summary(glm_modeltrain))[,4])
names(model_Train) = "GLM Training Set"
#TestDataSet
glm_modeltest <- glm(emp_test$Attrition~.,emp_test[,c(cols.CatGLM)], family = binomial)
model_Test = data.frame(coef(summary(glm_modeltest))[,4])
names(model_Test) = "GLM Test Set"
#AllData
glm_modelAll <- glm(emp_result$Attrition~.,emp_result[,c(cols.CatGLM)], family = binomial)
model_All = data.frame(coef(summary(glm_modelAll))[,4])
names(model_All) = "GLM All Set"
# Table consolidated
GLM_dataset <-cbind(model_Train, model_Test,model_All)
...
* ### *Detailed Models Based on Job Description*

#The business is also interested in learning about any job role specific trends that may exist in the data set (e.g., "Data Scientists have the highest job satisfaction"). Also created a kable table to show significance for Gender and Martial status by job.
