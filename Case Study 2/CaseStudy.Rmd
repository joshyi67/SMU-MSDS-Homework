...
dfTest <- read.csv("E:/Documents/School/MSDS 6306/Case Study 2/CaseStudy2Validation.csv", na.strings = "Null")
#Grant Path
...
dfTest2 <- fastDummies::dummy_cols(dfTest) # Create Dummy
...
# KNN
set.seed(1)
fit2 = class::knn(dfTrain2[,c(Col.KNN)],dfTest2[,c(Col.KNN)],dfTrain2$Attrition, k= 3)
fit = train(Attrition~., data=dfTrain2[,c(cols.CatKNN)], method="knn")

dfTest$Predictions = factor(fit2) # creating a new column for knn prediction
#dfVal$Predictions = predict(fit2,dfTest2)

#dfPreds = data.frame(dfVal$ID,dfVal$Attrition,dfVal$Predictions)
dfPreds = confusionMatrix(table(factor(fit2), dfTest$Attrition))
dfPreds
