...
Because of the data that we were able to analyze as part of the Par Plots above, we developed 2 Heat Maps and Correlations and Distribtuion Matrix to take a deeper dive in the dataset.
...
#### *Sginficant Key Factor attributing to Attirtion*

Showing the significance, p-values, for each variable. This is the base information to understand the key contirbuters that affect attirtion. This data will be used through our data science work so we can utitlize the following variables for the models we build.
...
Learning about any job role specific trends that may exist in the data set is key becayse it tells us which variables are significant by job.  This data can be used to identify what affects will contribute to attrition rate by job. Any value that is < 0.5 is signficant
...
pred_HRAttrition = confusionMatrix(table(factor(Job_HRTest$HR_Pred), Job_HRTest$Attrition))
...
pred_HRAttrition = confusionMatrix(table(Job_MDTest$MD_Pred, Job_MDTest$Attrition))
...
pred_RSAttrition = confusionMatrix(table(factor(Job_RSTest$RS_Pred),Job_RSTest$Attrition)) # running confusion Matrix
...
pred_LTAttrition = confusionMatrix(table(factor(Job_LTTest$LT_Pred),Job_LTTest$Attrition)) # running confusion Matrix
...
pred_SEAttrition = confusionMatrix(table(factor(Job_SETest$SE_Pred), Job_SETest$Attrition)) # running confusion Matrix at k = 3
...
pred_SRAttrition = confusionMatrix(table(factor(Job_SRTest$SR_Pred), Job_SRTest$Attrition)) # running confusion Matrix at k = 3
...
pred_HealthAttrition = confusionMatrix(table(Job_HealthTest$Health_Pred, Job_HealthTest$Attrition)) # running confusion Matrix at k = 3
```
#### *KNN Weighted*
Running the Weighted KNN model using the training and test data set. The Weighted KNN model came out to have a higher accuracy rate of 84.4 than the KNN which was 84%. Additional, the plot below shows the optinal K which is 30.
...
Adding to the the Logistric Regression model above, Logistic Regression Using Lasso was used for automated feature selection. As part of this model, we disovered that only 6 variables predict at 86% vs general logistic regression of 87% that utilizes all fields in the dataset.  Becaue the Logistic_Regression Using Lasso is more efficient, our recmmendatino is to run the logistic regrssion model using LASSO because it is more efficent and only 1% less prediction.
...
##### Summary Explanation:
The following table outlines the four different models that were used in accurcy order.  As you can tell, logestic regression was the most accurate; however, we recommend using the logistic regression using lasso because it is more efficient. 
...
```{r Logistic_Regression_Lasso Part 2}
#Begin Logistic Regression with Lasso
#convert training data to matrix format
set.seed(123)
x <- model.matrix(emp_train$Attrition~.,emp_train[,c(cols.CatGLM)])
...
train.reduce$Attrition <- emp_train$Attrition
#names(train.reduce)
#Assess Model

glm.manual <- glm(train.reduce$Attrition~.,data = train.reduce, family = "binomial", control = list(maxit = 100))
summary(glm.manual)

#Remove N/A
#names(train.reduce)
# Remove N/A Fields that Do Not Help Model
train.reduce = train.reduce[,c(-21:-28)]

glm.final <- glm(train.reduce$Attrition~.,data = train.reduce, family = "binomial", control = list(maxit = 100))
#glm.final <- glm(train.reduce$Attrition~.,data = train.reduce, family = "binomial")
summary(glm.final)
...
# Test Model
test$final.prob <- predict.glm(glm.final,emp_test[,-3],type="response")
test$final.predicted <- ifelse(test$final.prob>.5,"Yes","No")
confusionMatrix(table(emp_test$final.predicted, emp_test$Attrition))
...
### *Logistic_Regression Using Lasso*
Adding to the the Logistric Regression model above, Logistic_Regression Using Lasso was use for automated feature selection. As part of this model, we disovered that only 6 variables predict at 86% vs general logistic regression of 87% that utilizes all fields in the dataset.  Becaue the Logistic_Regression Using Lasso is more efficiency, our recmmendatino is to run the logistic regrssion model using LASSO because it is more efficent and only 1% less prediction.
...
##### Summary Explanation:
The following table outlines the four different models that were used in accurcy order.  As you can tell, logestic regression was the most accurate; howqever, we recommend using the logistic regression using lasso because it is more efficient. 
...
```
>>>>>>> 43f75e3c876ca307d29c6698d8af2820fb131da5
